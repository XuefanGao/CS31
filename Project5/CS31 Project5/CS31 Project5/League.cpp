...
BaseballTeam League::mostOverallLosses(BaseballTeam t1, BaseballTeam t2, BaseballTeam t3) {
    int l1 = t1.getHomeLosses() + t1.getAwayLosses();
    int l2 = t2.getHomeLosses() + t2.getAwayLosses();
    int l3 = t3.getHomeLosses() + t3.getAwayLosses();
    if (l1 >= l2 && l1 >= l3) {
        return t1;
    }
    if (l2 >= l1 && l2 >= l3) {
